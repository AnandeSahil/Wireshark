...
main(int argc,char **argv)
{
	...
	if((fp=fopen("outdata_q2e.txt", "w")) == NULL) {
	...
	while (currpos < InLen){
		...
		if( isgetTCPIP(pcktbuf, &size_ip, &size_tcp,fp)){
			...
			if (i_packet_size != 0)
			{
				...
				{
					int count = 0;
					while(count < i_packet_size)
					{
						fprintf(fp, "%c", *(payload + count));		//Printing into the file byte by byte.
						count++;
					}
				}
			}

			//fprintf(fp, "%s",payload);           //Using this, it prints all the padding and garabage data along with file content
		}  // isgetTCPIP
